...
class RoPE(nn.Module):
    ...
    def apply_rotary_embeddings(self, xq: torch.Tensor, xk: torch.Tensor, device:str):
        xq_complex = torch.view_as_complex(xq.float().reshape(*xq.shape[:-1], -1, 2))
        xk_complex = torch.view_as_complex(xk.float().reshape(*xk.shape[:-1], -1, 2))

        xq_rotated = xq_complex * self.freqs_complex
        xk_rotated = xk_complex * self.freqs_complex
...
if __name__ == '__main__':
    ...
    xq_out, xk_out = rope.apply_rotary_embeddings(xq, xk, 'cuda')
