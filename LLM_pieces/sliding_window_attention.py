...
def create_dynamic_block_mask(sliding_window_causal, q_len=1024, kv_len=1024, device='cuda'):
    # B,H set to None means that the mask is broadcasted for those dimentions as it doesn't require any calculation anyway
...
if __name__=='__main__':
    ...
    query = torch.rand((1,1,1024,128), device='cuda', dtype=torch.bfloat16)
    key = torch.rand((1,1,1024,128), device='cuda', dtype=torch.bfloat16)
    value = torch.rand((1,1,1024,128), device='cuda', dtype=torch.bfloat16)
    ...
    value_ = torch.rand((1,1,512,128), device='cuda', dtype=torch.bfloat16)
    
    block_mask = create_dynamic_block_mask(sliding_window_causal, 512, 512)
    out = flex_attention(query_, key_, value_, block_mask=block_mask, score_mod=softcap)
    print(out[0,0,:8,:8])

    #static
    static_mask = create_static_block_mask(sliding_window_causal, 1024, 1024)
